...
import { formatARS, formatPercent, colorDeCategoria } from '../../lib/datos';
import { CATEGORIA_LABELS } from '../../types/producto';
import type { CategoriaProducto } from '../../types/producto';
import type { IngresoPorCategoria } from '../../lib/datos';

interface GraficoCategoriasProps {
  data: ReadonlyArray<IngresoPorCategoria>;
}
...
function TooltipContent({ active, payload }: TooltipContentProps) {
  ...
  if (!row) return null;
  const color = colorDeCategoria(row.categoria);
  return (
    <div className="rounded-md border border-dash-line bg-dash-panel-2 px-3 py-2 text-xs shadow-lg">
      <div className="mb-1 flex items-center gap-2">
        ...
        <span className="font-medium text-dash-ink">{CATEGORIA_LABELS[row.categoria]}</span>
      </div>
      <div className="tabular text-base font-semibold text-dash-ink">{formatARS(row.ingreso)}</div>
      <div className="mt-0.5 text-[10px] uppercase tracking-wide text-dash-ink-2">
        {formatPercent(row.participacion, { signed: false })} del ingreso
      </div>
    </div>
  );
}
...
export default function GraficoCategorias({ data }: GraficoCategoriasProps) {
  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
      <div role="img" aria-label={ariaLabel} className="relative h-56 w-full shrink-0 sm:h-64 lg:h-56 lg:basis-[58%]">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            >
              {data.map((d) => (
                <Cell key={d.categoria} fill={colorDeCategoria(d.categoria as CategoriaProducto)} />
              ))}
            </Pie>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-[10px] uppercase tracking-wider text-dash-ink-3">Ingreso total</span>
          <span className="mt-1 text-xl font-semibold tabular-nums text-dash-ink sm:text-2xl">{formatARS(totalIngreso)}</span>
          <span className="mt-0.5 text-[10px] uppercase tracking-wider text-dash-ink-3">{data.length} categorías</span>
        </div>
      </div>
      <ul className="grid grid-cols-2 gap-x-4 gap-y-3 lg:basis-[42%] lg:grid-cols-1">
        {data.map((d) => {
          const color = colorDeCategoria(d.categoria as CategoriaProducto);
          return (
            <li key={d.categoria} className="flex items-baseline justify-between gap-3 border-b border-dash-line pb-2 last:border-b-0 lg:last:border-b">
              ...
              <span className="flex items-baseline gap-2 tabular-nums">
                <span className="text-[10px] uppercase tracking-wider text-dash-ink-3">{d.participacion.toFixed(1)}%</span>
                <span className="text-[12px] font-semibold text-dash-ink">{formatARS(d.ingreso)}</span>
              </span>
            </li>
          );
        })}
